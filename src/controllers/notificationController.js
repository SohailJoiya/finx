...
exports.listUserNotifications = async (req, res) => {
  ...
  const notes = await Notification.find({ $or: [{ user: userId }, { user: null }] }).sort({ createdAt: -1 });
  res.json(notes);
};
...
exports.createNotificationForUser = async (req, res) => {
  try {
    ...
    if (!userId || !title || !message) return res.status(400).json({ message: 'Missing fields' });
